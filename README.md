...
The Codex plugin includes a session-start hook plus declaration enforcement for `apply_patch` and common shell-based file writes. Declaration files are scoped per client/session and workspace instead of sharing one global `/tmp/claude-declaration.json`.
